<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-3xl font-bold text-gray-900 dark:text-white">Pengeluaran Saya</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Kelola pencatatan pengeluaran pribadi (Petty Cash)</p>
            </div>
            <a href="{{ route('kasir.expenses.create') }}"
    ...
    <div class="space-y-6">
        {{-- Filter Section --}}
        <x-card>
            <form method="GET" action="{{ route('kasir.expenses.index') }}" class="flex flex-wrap items-end gap-4">
                <div>
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('kasir.expenses.index') }}" class="px-4 py-2 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 transition-colors">Reset</a>
                </div>
            </form>
        </x-card>
                        ...
                        <x-table-row>
                            ...
                            <x-table-cell>
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('kasir.expenses.edit', $expense) }}" class="inline-flex items-center px-3 py-1 text-sm font-medium text-blue-600 transition-colors rounded-lg bg-blue-50 dark:bg-blue-900/30 dark:text-blue-400 hover:bg-blue-100">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    </a>
                                    <form action="{{ route('kasir.expenses.destroy', $expense) }}" method="POST" class="inline" onsubmit="return confirm('Hapus pengeluaran ini?')">
                                        @csrf @method('DELETE')
