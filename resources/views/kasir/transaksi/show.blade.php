    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-end space-x-3">
            ...
            @if($transaksi->status !== 'cancelled')
                <form action="{{ route('kasir.transaksi.batalkan', $transaksi->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan transaksi ini? Stok akan dikembalikan.')">
                    @csrf
                    @method('PATCH')
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-sm sm:w-full">
                ...
                <div class="grid grid-cols-3 gap-2 px-6 py-4 bg-gray-50 border-t">
                    ...
                    <button onclick="window.location.href='{{ route('kasir.transaksi.index') }}'" class="flex items-center justify-center px-4 py-3 text-sm font-semibold text-gray-700 transition-colors bg-white border border-gray-300 rounded shadow-sm hover:bg-gray-100">
                        📋 Riwayat
                    </button>
