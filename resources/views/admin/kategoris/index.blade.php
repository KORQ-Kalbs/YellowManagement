<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse(\App\Models\Kategori::withCount('products')->get() as $kategori)
            <x-card>
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $kategori->nama_kategori }}</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">{{ $kategori->products_count }} products</p>
                    </div>
                    <button onclick="editCategory({{ $kategori->id }}, '{{ $kategori->nama_kategori }}')" class="p-2 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors">
            ...
            <form id="category-form" action="{{ route('admin.kategoris.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="modal-name" name="nama_kategori" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white" required>
                    </div>
                </div>
